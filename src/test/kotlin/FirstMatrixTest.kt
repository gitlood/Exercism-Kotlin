...
class FirstMatrixTest {
    ...
    fun `extract row from one number matrix`() {
        ...
        assertEquals(expectedRow, FirstMatrix(matrixAsString).row(rowIndex))
    }
    ...
    fun `extract column from one number matrix`() {
        ...
        assertEquals(expectedColumn, FirstMatrix(matrixAsString).column(columnIndex))
    }
